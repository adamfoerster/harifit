...
export class ObjectiveService {
  ...
  get objectives$(): Observable<Objective[]> {
    return this.db.collection<Objective>('objectives').valueChanges().pipe(first());
  }

  get myObjective$(): Observable<Objective> {
    return this.db.collection('objectives').doc<Objective>(this.core.user.email).valueChanges().pipe(
      first()
    );
  }
  ...
  setMonthProgress(weight) {
    ...
    this.myObjective$.subscribe(obj => {
      ...
      this.db
        ...
        .then(_ => this.core.loading = false);
    });
  }
}
